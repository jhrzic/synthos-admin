...
// The model REGISTRY below is a seat roster: one entry per Admin tab/agent
// seat, not a catalog of models. That distinction was lost — each entry's
// `name` carried a model version ("Claude 3.7 Sonnet / Opus", "Gemini 3.7 /
// 3.6 Flash"), there is no modelId field at all, and so a provider
// structurally WAS one model. Updating a model meant editing a label, which is
// why a stale Claude version outlived its relevance on screen.
//
// Model identity now lives in lib/model-catalog.ts, where providers and models
// are separate and one provider may expose several. Names here identify the
// SEAT and its vendor; they no longer assert a version.
//
// `latency` and `tokensPerSec` were invented: 145ms and 84 tok/s sat on
// providers whose own status field read `requires_key`, meaning they had never
// been called. They are zero, and the UI renders zero as UNKNOWN. Real
// latency is measured per call and recorded in the provider ledger.
// INITIAL_MODELS was a UI-only model array (seat names, model versions and
// per-token prices typed into the client). It is gone: every model the UI shows
// now comes from the persisted local model registry (GET /api/registry/models,
// lib/registry), which is populated only from versioned provider manifests.
