...
**EXAMPLE TEMPLATE — illustrative, not real research.** Scores, TAM/SAM/SOM and financial
projections below are hand-written placeholder content showing the output format; no live scrape,
market analysis, or verification produced them.
...
export const StartupIdeaGeneratorView: React.FC<StartupIdeaGeneratorViewProps> = ({
  ...
  // Pass X / Workstream F — this previously defaulted to SAMPLE_GENERATED_IDEAS[0], so the very
  // first thing a user saw on this screen (before touching anything) was a fully fabricated
  // "AgentCache" startup thesis — a 96/100 viability score, $4.8B TAM, "240+ trending agent
  // repos" — with no indication it was canned example content rather than something the system
  // had generated or researched. Starts empty; the real empty state below already exists.
  ...
  const handleTriggerScrape = async () => {
    ...
    setCurrentStage(1);
    // Pass X / Workstream F — this previously claimed "Scout is crawling live GitHub trends,
    // Product Hunt launches & arXiv preprints" during a plain setTimeout with no network call of
    // any kind. No live scrape happens anywhere in this flow — only the one real onSendQuery()
    // call below produces genuine model output; everything else is a template filled with
    // illustrative placeholder numbers. Progress copy now says so.
    setStageProgressText('Drafting Scout-angle framing (illustrative template, not a live scrape)...');
    ...
    try {
      // Stage 1: template drafting delay (no real scrape happens)
      await new Promise(r => setTimeout(r, 900));
      setCurrentStage(2);
      setStageProgressText('Filling in illustrative TAM/SAM/SOM and unit-economics placeholders (not computed from real data)...');

      // Stage 2: template drafting delay
      await new Promise(r => setTimeout(r, 900));
      setCurrentStage(3);
      setStageProgressText('Drafting illustrative technical and go-to-market sections...');

      // Stage 3: template drafting delay
      await new Promise(r => setTimeout(r, 800));
      setCurrentStage(4);
      setStageProgressText('Requesting the real executive-summary paragraph from the model...');
...
**AI-DRAFTED BRAINSTORMING TEMPLATE — not real research.** Only the Executive Summary section
below is real model output (from a live query); every score, TAM/SAM/SOM figure, and financial
projection elsewhere in this document is an illustrative placeholder, not a computed or verified
value.
**Vault Folder**: \`Startup-Theses/\`
**Domain**: \`${domainName}\`
...
#startup-thesis #${domainName.toLowerCase().replace(/[^a-zA-Z0-9]/g, '-')} #scout #analytics #dev #reach #hermes #obsidian`
  ...
  const handleDispatchTelegram = () => {
    ...
    // Pass X / Workstream F — these previously said "Identified" and "validated", asserting real
    // research/verification had occurred. It hadn't: these are illustrative placeholder numbers
    // from an AI-drafted template. Wording now matches what actually happened.
    onSendTelegramMessage(
      'scout',
      `/thesis AI-drafted startup thesis template: "${generatedIdea.title}" (illustrative score: ${generatedIdea.viabilityScore}/100, illustrative TAM: ${generatedIdea.tam}, not real market research). Saved to [[Startup-Theses/${generatedIdea.title.replace(/[^a-zA-Z0-9-]/g, '')}]].`
    );
  ...
  return (
    <div className="space-y-6 pb-12">
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          {generatedIdea ? (
            <div className="bg-[#070810] border border-[#1C2036] rounded-2xl shadow-2xl overflow-hidden space-y-0">
              {/* Pass X / Workstream F — every score, TAM/SAM/SOM figure and financial projection
                  in this card is an illustrative placeholder from a template, not real research.
                  Only the Executive Summary paragraph may reflect real model output. */}
              <div className="px-6 pt-4 pb-2 bg-[#1D1706] border-b border-[#E8A845]/30 flex items-center gap-2 text-[11px] text-[#E8A845]">
                <AlertCircle className="w-3.5 h-3.5 shrink-0" />
                <span>Illustrative brainstorming template — scores and financial figures below are placeholders, not real market research or validated data.</span>
              </div>
                  ...
                  <div className="space-y-3 text-xs font-mono">
                    <div className="p-3.5 bg-[#040409] border border-[#161828] rounded-xl space-y-2">
                      <div className="text-[10px] text-[#20B2AA] font-bold uppercase flex items-center gap-1.5">
                        ...
                        <span>SCOUT ANGLE (illustrative — no live scrape ran)</span>
                      </div>
            ...
            <div className="text-center py-24 bg-[#070810] border border-[#181B2E] rounded-2xl text-[#6A7097] font-mono text-xs space-y-2">
              ...
              <div>Draft an AI-assisted startup thesis template. Illustrative brainstorming output — not real market research or a verified analysis.</div>
            </div>
          )}
        </div>
      </div>
      ...
      {/* Example Thesis Templates — Pass X / Workstream F: these are hand-authored example
          output, not real research and not the product of any real "cron sweep" (this app has
          no scheduler — CLAUDE.md is explicit that no such thing exists). Labeled honestly. */}
      <div className="bg-[#080913] border border-[#181B2E] rounded-2xl p-6 space-y-4 shadow-xl">
        <div className="flex items-center justify-between border-b border-[#161828] pb-3">
          <div className="space-y-0.5">
            <h3 className="text-base font-bold text-white font-['Space_Grotesk']">
              Example Thesis Templates
            </h3>
            <p className="text-xs text-[#8E94B8]">
              Illustrative example output — hand-written templates showing the format, not real automated research.
            </p>
          </div>

          <span className="text-[10px] font-mono text-[#A5A2FF] bg-[#615EFF]/15 px-2.5 py-1 rounded border border-[#615EFF]/30">
            {SAMPLE_GENERATED_IDEAS.length} EXAMPLE TEMPLATES
          </span>
        </div>
