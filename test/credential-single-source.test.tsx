...
describe('2. A BROWSER VALUE can never produce a configured/connected state', () => {
  ...
  // Integrated onto the checkpoint's stricter rule: an ALLOWLIST. Only the
  // non-secret SERVER_MANAGED_KEY marker survives; every provider value is
  // dropped, including one a future provider row adds. A blocklist of named
  // providers (the earlier shape of this fix) leaks the next one anyone adds.
  it('the browser persists no provider key value — only the server-managed marker', () => {
    expect(APP).toContain("value === 'SERVER_MANAGED_KEY' ? value : undefined");
    expect(APP).not.toContain('BROWSER_UNPERSISTED_PROVIDER_KEYS');
  });

  it('elevenlabs is NOT exempt: a browser-used key is still not written to storage', () => {
    // JarvisView really does send this as an xi-api-key header, so it has to
    // be re-entered after a reload. That cost was accepted: no plaintext
    // secret sits in localStorage. Pinned so the trade-off is deliberate.
