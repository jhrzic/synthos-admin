...
describe('2. TRUTHFUL CONFIGURATION STATE — NOT_CONFIGURED is reported, never assumed away', () => {
  ...
  it('the capability registry reports model.openai NOT_CONFIGURED, with a reason naming the real evidence', async () => {
    delete process.env.OPENAI_API_KEY;
    deleteModelCredential('openai');
    const cap = (await listCapabilities()).find((c) => c.key === 'model.openai');
    expect(cap).toBeTruthy();
    expect(cap!.status).toBe('NOT_CONFIGURED');
    expect(cap!.reason).toContain('OPENAI_API_KEY');
    // Never claims availability it cannot back.
  ...
  it('once a key exists the capability flips to AVAILABLE — and AVAILABLE still only ever means "a credential resolves", never "the provider is up"', async () => {
    process.env.OPENAI_API_KEY = 'sk-test-key-for-capability-resolution-only';
    try {
      const cap = (await listCapabilities()).find((c) => c.key === 'model.openai');
      expect(cap!.status).toBe('AVAILABLE');
      expect(cap!.reason).toContain('proven at call time');
