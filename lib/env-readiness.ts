...
export const ENV_VAR_SPECS: EnvVarSpec[] = [
  ...
  { variable: 'SYNTHOS_LOCAL_RUNTIME_BASE_URL', subsystem: 'Model Registry (local runtime route)', requiredFor: 'Base URL of a self-hosted OpenAI-compatible runtime (loopback only; no credential is sent). Read through the local-runtime provider manifest (baseUrlEnvVar). Unset: http://127.0.0.1:11434/v1 (the only approved host).', requirement: 'OPTIONAL', secrecy: 'NON_SECRET', validate: isHttpUrl },
  { variable: 'SYNTHOS_SHUTDOWN_DRAIN_MS', subsystem: 'Service lifecycle (graceful shutdown)', requiredFor: 'Upper bound, in milliseconds, on how long a SIGTERM/SIGINT shutdown waits for in-flight executions before settling them durably (never-sent reservations released, sent-but-unanswered calls UNKNOWN, interrupted tasks resumable or RECONCILING) and exiting. Unset: 6000. Capped at 60000.', requirement: 'OPTIONAL', secrecy: 'NON_SECRET' },
  { variable: 'SYNTHOS_OLLAMA_MODELS_DIR', subsystem: 'Model Registry (local model substance)', requiredFor: 'Directory of the local Ollama models store, read (never written) to verify a local route\'s approved manifest/config/weights digests before every dispatch (lib/registry/substance.ts). Unset: ~/.ollama/models.', requirement: 'OPTIONAL', secrecy: 'NON_SECRET' },
  ...
  { variable: 'SYNTHOS_SCHEDULER_DISABLED', subsystem: 'Scheduler', requiredFor: 'Set to 1 to start NO scheduler in this process (nothing dispatches on a timer). For verification copies and any non-canonical instance; the canonical control plane leaves it unset.', requirement: 'OPTIONAL', secrecy: 'NON_SECRET', validate: (v) => v === '' || v === '1' || v === '0' },
